...
export class NewPostComponent implements OnInit {
    ...
    if (this.postId == null)
      ...
      this.postService.getPost(this.postId).subscribe(result => {
        ...
        this.editorForm = new FormGroup({
          ...
          'editor': new FormControl(this.newPost.postContent, Validators.required),
          'isActive': new FormControl(this.newPost.status),
          'allowComments': new FormControl(this.newPost.allowComment),
          'category': new FormControl(this.newPost.category.id, Validators.required)
        });
        this.spinner.hide();
      },
        error => {
  ...
  onSave() {
    if (this.postId == null) {
      this.userService.getUser(localStorage.getItem(TOKEN_USER)).subscribe(result => {
        this.newPost.postName = this.editorForm.get('title').value;
        this.newPost.status = this.editorForm.get('isActive').value;
        this.newPost.allowComment = this.editorForm.get('allowComments').value;
        this.newPost.postDate = new Date();
        this.newPost.postContent = this.editorContent;
        this.postService.addPost(this.newPost, result.id, this.editorForm.get('category').value)
          .subscribe(
            () => {
              this.editorContent = "";
      ...
      this.userService.getUser(localStorage.getItem(TOKEN_USER)).subscribe(result => {
        ...
        this.postService.putPost(this.newPost, result.id, this.editorForm.get('category').value)
          .subscribe(() => {
            this.editorContent = "";
            this.editorForm.reset();
            this.router.navigate(['/home/dashboard/myposts']);
