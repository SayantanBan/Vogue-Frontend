...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.categorySubscription = this.dataStorageService
      .fetchCategories()
      .subscribe();
    this.themeService.isDarkTheme.subscribe((theme) => {
      this.isDark = theme;
      if (theme) localStorage.setItem("dark-theme", "true");
      else {
    ...
    const options = {
      strings: [
        "My project includes: <i>Angular</i> as frontend",
        "My project includes: <strong>Spring Boot</strong> as backend",
        "Many third party libraries like: <i><strong>typed.js</stronG></i>",
      ],
      typeSpeed: 100,
      backSpeed: 0,
  ...
  checkUserSession(): void {
    if (localStorage.getItem(TOKEN_NAME)) {
      if (!this.userService.checkToken()) this.isLogged = true;
      else {
        this.isLogged = false;
        this.logout();
      }
...
export class LoginDialog implements OnInit {
  ...
  signingIn: string =
    '<i class="fa fa-circle-o-notch fa-spin" aria-hidden="true"></i> Signing In';
  ...
  onSubmit() {
    ...
    this.authenticationService
      .login(this.credential.username, this.credential.password)
      .subscribe(
        (result) => {
          if (result) {
  ...
  ngOnInit() {
    if (this.data.isDark)
      ...
      this.overlayContainer
        .getContainerElement()
        .classList.remove("unicorn-dark-theme");
    }
    this.userService.logout();
    this.isSignUp = false;
  }
